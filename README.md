...
fishow是mqantserver的初始化项目，提供gopath和proxy配置文件。编译前使用git bash的source命令使gopath生效，然后执行go install server编译，执行./bin/server启动。项目以包含mqantserver所需依赖，无需再次添加，详细请参考以下mqantserver项目介绍。
